...
class SimpleBipedGaitProblem:
    ...
    def createWalkingProblem(       
        ...
        :param stepX: step length in x
        :param stepY: step offset in y
        :param stepYaw: step rotation in yaw (rad)
        ...
        rfOri0 = self.rdata.oMf[self.rfId].rotation                             # right foot world orientation
        lfOri0 = self.rdata.oMf[self.lfId].rotation                             # left foot world orientation
    ...
    def createFootstepModels(
        ...
        :param stepX: step length in x-direction
        :param stepY: step offset in y-direction
        :param stepYaw: step rotation (yaw angle in radians)
        :param stepHeight: step height
        ...
        for k in range(numKnots):
            ...
            for i, p, r in zip(swingFootIds, feetPos0, feetOri0):
                ...
                dx = stepX * (k + 1) / numKnots
                dy = stepY * (k + 1) / numKnots
                dyaw = stepYaw * (k + 1) / numKnots
                
                R = r @ pinocchio.rpy.rpyToMatrix(0.0, 0.0, dyaw)
                dp = np.array([dx, dy, dz])
                tref = p + dp
                swingFootTask += [[i, pinocchio.SE3(R, tref)]]
            
            comTask = (
                np.array([stepX * (k + 1) / numKnots, stepY * (k + 1) / numKnots, 0.0]) * comPercentage
                + comPos0
            )
            footSwingModel += [
                self.createSwingFootModel(
        ...
        comPos0 += [stepX * comPercentage, stepY * comPercentage, 0.0]
        for p in feetPos0:
            p += [stepX, stepY, 0.0]
        for r in feetOri0:
            r = r @ pinocchio.rpy.rpyToMatrix(0.0, 0.0, stepYaw)
    ...
    def createSwingFootModel(
        ...
        for i in supportFootIds:
            cone = crocoddyl.WrenchCone(self.Rsurf, self.mu, self.foot_dim) # rotation of surface, friction coeff, foot length and width
    ...
    def createPseudoImpulseModel(self, supportFootIds, swingFootTask):
        ...
        costModel = crocoddyl.CostModelSum(self.state, nu)
        for i in supportFootIds:
            cone = crocoddyl.WrenchCone(self.Rsurf, self.mu, self.foot_dim)
            wrenchResidual = crocoddyl.ResidualModelContactWrenchCone(
                self.state, i, cone, nu, self._fwddyn
            )
            wrenchActivation = crocoddyl.ActivationModelQuadraticBarrier(
