...
def verify_joint_order(model, expected_dof_list=None) -> list:
    ...
    if expected_dof_list is not None:
        if len(model_dof_list) != len(expected_dof_list):
            raise ValueError(
                f"[verify] Joint count mismatch:\n"
                f"  pinocchio model  : {len(model_dof_list)} joints\n"
                f"  expected_dof_list: {len(expected_dof_list)} joints\n"
                f"  Model order: {[n for n, _ in model_dof_list]}"
            )
        mismatches = []
        for i, ((m_name, m_dof), (e_name, e_dof)) in enumerate(
            zip(model_dof_list, expected_dof_list)
        ):
